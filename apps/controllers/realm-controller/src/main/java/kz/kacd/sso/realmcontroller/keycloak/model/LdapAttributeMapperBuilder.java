...
public class LdapAttributeMapperBuilder {
    ...
    public LdapAttributeMapperBuilder(ComponentRepresentation parent) {
    ...
    public LdapAttributeMapperBuilder withAttributeMapping(
        ...
        config.put(ALWAYS_READ_VALUE, List.of("true"));
        config.put(REQUIRED_IN_LDAP, List.of(Boolean.valueOf(required).toString()));
        config.put(LDAP_ATTR, List.of(from));
        config.put(READ_ONLY, List.of(Boolean.valueOf(readOnly).toString()));
        config.put(USER_MODEL_ATTR, List.of(to));
