...
const Button = ({ children, type, severity, disabled, onClick }: ButtonProps) => (
    <div className="mt-6">
        <button
            type={type ?? "button"}
            onClick={onClick ?? (() => { })}
            className={`w-full px-4 py-2 tracking-wide text-white transition-colors duration-200 transform rounded-md disabled:bg-gray-700 focus:outline-none focus:ring ${severityStyles(severity)}`}
            disabled={disabled}
        >
            {children}
        </button>
