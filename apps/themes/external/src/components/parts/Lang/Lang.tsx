...
const Lang = ({ current, langs, i18n }: { current: string, langs: any[], i18n: I18n }) => {
    ...
    const Choice = ({ label, url }: { label: string, url: string }) => (
        <div className="cursor-pointer w-full border-gray-100 rounded-t border-b text-gray-600">
            <div className="flex w-full items-center p-2 pl-2 border-transparent bg-white border-l-2 relative hover:text-primary hover:border-primary">
                <div className="w-full items-center flex">
                    <a href={url} className="mx-2 leading-6 text-xs">{label}</a>
                </div>
            </div>
        </div>
    )
            ...
            <div className="flex flex-col items-center relative">
                <div className="w-full">
                    <div className="my-1 bg-transparent p-1 flex rounded">
                        ...
                        <input value={msgStr(selected.languageTag)} onChange={() => {}} className="p-1 px-2 appearance-none outline-none w-full bg-transparent text-xs text-gray-800" readOnly />
                        <div>
